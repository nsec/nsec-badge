 ...
 *
 * Uses heap management code adapted from Babeltrace 2's prio-heap.c, itself
 * MIT licensed.
 *
 * Copyright 2011 Mathieu Desnoyers <mathieu.desnoyers@efficios.com>
 */

#ifndef NSEC_SCHEDULING_SCHEDULER_HPP
...
template <unsigned int> class scheduler;

class task
{
    template <unsigned int> friend class scheduler;
...
class periodic_task : public task
{
  ...
  public:
    ...
    /* Indicate that this task should no longer be scheduled after the current
     * execution. */
    void kill() noexcept
    {
        _killed = true;
...
template <unsigned int max_scheduled_tasks> class scheduler
{
  public:
     ...
     * allowing the MCU to sleep when the next task is sufficiently far away.
     */
  ...
  private:
    ...
    class task_heap
    {
      ...
      private:
        ...
        bool _task_should_run_before(const task &a,
                                     const task &b) const noexcept
        {
...
#endif /* NSEC_SCHEDULING_SCHEDULER_HPP */
