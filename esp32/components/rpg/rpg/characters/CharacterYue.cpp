...
void CharacterYue::post_render(Viewport &viewport)
{
...
void CharacterYue::render(Viewport &viewport)
{
    if (current_mode == Mode::standing) {
        auto screen = viewport.to_screen(get_coordinates());
        graphics_draw_from_library(LIBRARY_IMAGE_YUE_STANDING, screen.x(),
                                   screen.y());
        ...
        if (timer.has_expired(get_animation_step())) {
            ...
            case 0:
                ...
                timer.start(get_animation_step(), 29, 13);
            ...
            default:
                ...
                timer.start(get_animation_step(), 43, 17);
            }
        }
        ...
        if (timer.has_expired(get_animation_step())) {
            current_mode = Mode::standing;
            timer.start(get_animation_step(), 43, 19);
        }
    }
