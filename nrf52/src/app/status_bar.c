...
void nsec_status_set_ble_status(status_bluetooth_status status) {
    ...
    if(ble_status != STATUS_BLUETOOTH_ON) {
        ...
        gfx_draw_line(x + ble_logo_bitmap.width, y,
            x, y + ble_logo_bitmap.height, DISPLAY_RED);
...
void nsec_status_set_battery_status(status_battery_state state)
{
    ...
    if(battery_state == STATUS_BATTERY_CHARGING) {
            gfx_draw_16bit_bitmap(BATTERY_POS, &battery_charging_bitmap, STATUS_BAR_BG_COLOR);
    } else {
            gfx_draw_16bit_bitmap(BATTERY_POS, &battery_bitmap, STATUS_BAR_BG_COLOR);
        switch (battery_state) {
            case STATUS_BATTERY_25_PERCENT:
                gfx_fill_rect(BATTERY_POS_X+1, BATTERY_POS_Y+1, 3, 6, DISPLAY_RED);
                break;
            case STATUS_BATTERY_50_PERCENT:
                gfx_fill_rect(BATTERY_POS_X+1, BATTERY_POS_Y+1, 6, 6, DISPLAY_YELLOW);
                break;
            case STATUS_BATTERY_75_PERCENT:
                gfx_fill_rect(BATTERY_POS_X+1, BATTERY_POS_Y+1, 9, 6, DISPLAY_GREEN);
                break;
            case STATUS_BATTERY_100_PERCENT:
                gfx_fill_rect(BATTERY_POS_X+1, BATTERY_POS_Y+1, 13, 6, DISPLAY_GREEN);
                break;
            default:
                gfx_fill_rect(BATTERY_POS_X+1, BATTERY_POS_Y+1, 0, 6, DISPLAY_GREEN);
                break;
        }
    }
    gfx_update();
