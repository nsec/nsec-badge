...
void nsec_gfx_effect_addNoise(uint8_t noise_amount) {
    for (int16_t y = 0; y < DISPLAY_HEIGHT; y++) {
        for (int16_t x = 0; x < DISPLAY_WIDTH; x++) {
            ...
            nsec_random_get(&r, 1);
            if (r < noise_amount) {
                // TODO do something in mode display instead of reversing the
                // color
...
void gfx_update(void) { display_update(); }
...
void gfx_draw_circle_helper(int16_t x0, int16_t y0, int16_t r,
                            uint8_t cornername, uint16_t colour) {
    int16_t f = 1 - r;
    int16_t ddF_x = 1;
    int16_t ddF_y = -2 * r;
    int16_t x = 0;
...
void gfx_draw_round_rect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r,
                         uint16_t colour) {
...
void gfx_fill_round_rect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r,
                         uint16_t colour) {
...
// void draw_round_rect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r,
//                     uint16_t colour) {
//    gfx_draw_fast_hline(x + r, y, w - 2 * r, colour);
//    gfx_draw_fast_hline(x + r, y + h - 1, w - 2 * r, colour);
//    gfx_draw_fast_vline(x, y + r, h - 2 * r, colour);
...
// void fill_round_rect(int16_t x, int16_t y, int16_t w, int16_t h, int16_t r,
//                     uint16_t colour) {
//    gfx_fill_rect(x + r, y, w - 2 * r, h, colour);
//    gfx_fill_circle_helper(x + w - r - 1, y + r, r, 1, h - 2 * r - 1, colour);
//    gfx_fill_circle_helper(x + r, y + r, r, 2, h - 2 * r - 1, colour);
...
void gfx_draw_16bit_bitmap(int16_t x, int16_t y, const struct bitmap *bitmap,
                           uint16_t bg_color) {
    display_draw_16bit_bitmap(x, y, bitmap->image, bitmap->width,
                              bitmap->height, bg_color);
}

void gfx_draw_bitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w,
...
void gfx_draw_bitmap_bg(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w,
...
void gfx_draw_xbitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w,
                      int16_t h, uint16_t color) {
    int16_t i, j, byteWidth = (w + 7) / 8;
        ...
        for (int8_t j = 0; j < 8; j++) {
            if (line & 0x1) {
                if (size == 1) { // default size
                    ...
                    gfx_fill_rect(x + (i * size), y + (j * size), size, size,
                                  color);
                }
            } else if (bg != color) {
                if (size == 1) // default size
...
void gfx_write(uint8_t c) {
    if (c == '\n') {
        ...
        gfx_draw_char(gfx_cursor_x, gfx_cursor_y, c, gfx_textcolor,
                      gfx_textbgcolor, gfx_textsize);
...
void gfx_putc(char c) { gfx_write((uint8_t)c); }
...
void gfx_set_text_size(uint8_t s) { gfx_textsize = (s > 0) ? s : 1; }
