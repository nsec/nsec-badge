...
void register_nsec_vendor_specific_uuid(){
    ...
    register_custom_vendor_uuid(&base_uuid, &TYPE_NSEC_UUID);
}

/*
 * Register the vendor specific 128 bit UUID of a custom vendor in the BLE stack to allow services and characteristics
 * to use a 16 bit UUID by setting ble_uuid_t.type to the type return by this function.
 */
void register_custom_vendor_uuid(ble_uuid128_t* vendor_uuid, uint8_t* vendor_uuid_type){
    APP_ERROR_CHECK(sd_ble_uuid_vs_add(vendor_uuid, vendor_uuid_type));
}
