...
void nsec_status_bar_ui_redraw(void) {
    gfx_fillRect(0, 0, 128, 8, SSD1306_BLACK);
    gfx_setCursor(0, 0);
    ...
    if(ble_status == STATUS_BLUETOOTH_ON) {
        gfx_drawBitmap(128 - 18, 0, ble_logo_bitmap, ble_logo_bitmap_width, ble_logo_bitmap_height, SSD1306_WHITE);
    }
    if(battery_state == STATUS_BATTERY_CHARGING) {
        gfx_drawBitmap(128 - 11, 0, battery_charging_bitmap, battery_charging_bitmap_width, battery_charging_bitmap_height, SSD1306_WHITE);
    }
    else {
        gfx_drawBitmap(128 - 11, 1, battery_bitmap, battery_bitmap_width, battery_bitmap_height, SSD1306_WHITE);
        uint8_t width;
        ...
        gfx_fillRect(128 - 10, 2, width, 3, SSD1306_WHITE);
    }
    gfx_update();
}
